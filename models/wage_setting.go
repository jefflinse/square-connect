...
func (m *WageSetting) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateJobAssignments(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *WageSetting) contextValidateJobAssignments(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.JobAssignments); i++ {

		if m.JobAssignments[i] != nil {
			if err := m.JobAssignments[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("job_assignments" + "." + strconv.Itoa(i))
				}
				return err
