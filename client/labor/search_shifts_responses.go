// Code generated by go-swagger; DO NOT EDIT.

package labor

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/jefflinse/square-connect/models"
)

// SearchShiftsReader is a Reader for the SearchShifts structure.
type SearchShiftsReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *SearchShiftsReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewSearchShiftsOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("unknown error", response, response.Code())
	}
}

// NewSearchShiftsOK creates a SearchShiftsOK with default headers values
func NewSearchShiftsOK() *SearchShiftsOK {
	return &SearchShiftsOK{}
}

/*SearchShiftsOK handles this case with default header values.

Success
*/
type SearchShiftsOK struct {
	Payload *models.SearchShiftsResponse
}

func (o *SearchShiftsOK) Error() string {
	return fmt.Sprintf("[POST /v2/labor/shifts/search][%d] searchShiftsOK  %+v", 200, o.Payload)
}

func (o *SearchShiftsOK) GetPayload() *models.SearchShiftsResponse {
	return o.Payload
}

func (o *SearchShiftsOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.SearchShiftsResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
