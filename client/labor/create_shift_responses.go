// Code generated by go-swagger; DO NOT EDIT.

package labor

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/jefflinse/square-connect/models"
)

// CreateShiftReader is a Reader for the CreateShift structure.
type CreateShiftReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *CreateShiftReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewCreateShiftOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("unknown error", response, response.Code())
	}
}

// NewCreateShiftOK creates a CreateShiftOK with default headers values
func NewCreateShiftOK() *CreateShiftOK {
	return &CreateShiftOK{}
}

/*CreateShiftOK handles this case with default header values.

Success
*/
type CreateShiftOK struct {
	Payload *models.CreateShiftResponse
}

func (o *CreateShiftOK) Error() string {
	return fmt.Sprintf("[POST /v2/labor/shifts][%d] createShiftOK  %+v", 200, o.Payload)
}

func (o *CreateShiftOK) GetPayload() *models.CreateShiftResponse {
	return o.Payload
}

func (o *CreateShiftOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.CreateShiftResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
